...
- `1000 lx / 6500 K`의 blue-enriched bright light가 `100 lx / 4000 K`의 일반 실내 조명보다 졸림, 부정 정서, 작업기억 저하를 더 잘 완화했다.
- 이 결과는 Mercury, Mars, Neptune의 각성·집중 계열 anchor로 사용한다.
...
- 이 결과는 Saturn, Earth의 낮은 workload/긴 세션 조명 anchor와 Mercury/Mars의 cool-bright 보정 근거로 사용한다.
...
- 이 결과는 Venus/Uranus의 warm-creative 계열과 Neptune/Mercury의 cool-focus 계열을 분리하는 핵심 근거다.
...
다이내믹 조명 패턴은 직접 lighting literature가 충분하지 않다. 그래서 패턴은 아래처럼 제한적으로 쓴다.
...
- 약 `6 breaths/min`의 paced breathing이 HRV 조절을 높였다.
- 이를 Pluto의 `resonance-breath` 패턴으로 변환하되, 문서와 payload에는 `inferred`라고 명시한다.
